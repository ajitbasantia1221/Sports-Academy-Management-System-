...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	echo "	
			...
			<h1>Location details of $tlid is:<h1><br /><br />";
	$sql="select * from tournament_loc where LOC_ID like '%$tlid%' ";

				$result = $con->query($sql);

			if ($result->num_rows > 0) {
		    echo "<table><tr><th>LOCATION_ID</th><th>LOCATION_NAME</th><th>LOCATION_AREA</th><th>TOUR_ID</th></tr>";
		   		    while($row = $result->fetch_assoc()) {
		        echo "<tr><td>" . $row["LOC_ID"]. "</td><td>" . $row["LOC_NAME"]. "</td><td>".$row["LOC_AREA"]."</td><td>".$row["TOUR_ID"]."</td></tr><br>";

		    }
		    echo "</table>";
		     echo "<button class=btn_Continue><a href='tour_loc.html'>Back</a></button>";
		} else {
		    echo "Incorrect LOCATION_ID <br>";
