...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	echo "	
			...
			<h1>User details of $inid is:</h1><br /><br />";
	$sql="select * from INSTRUCTOR where INS_ID like '%$inid%' ";

				$result = $con->query($sql);

			if ($result->num_rows > 0) {
		    echo "<table><tr><th>INS_ID</th><th>INS_NAME</th><th>COURSE_ID</th><th>INS_SALARY</th></tr>";
		   		    while($row = $result->fetch_assoc()) {
		        echo "<tr><td>" . $row["INS_ID"]. "</td><td>" . $row["INS_NAME"]. "</td><td>".$row["COURSE_ID"]."</td><td>".$row["INS_SALARY"]."</td></tr><br>";

		    }
		    echo "</table>";
		     echo "<button class=btn_Continue><a href='instructor.html'>Back</a></button>";
		} else {
		    echo "Incorrect INS_ID <br>";
		    echo "<a href='inssearch.html'>Try again</a>";
		}
		}
